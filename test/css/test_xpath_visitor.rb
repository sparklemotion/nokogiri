...
describe Nokogiri::CSS::XPathVisitor do
  ...
  def assert_xpath(expecteds, selector, ns: nil)
    asts = Nokogiri::CSS::Parser.new(selector, ns: ns).parse
    expecteds = Array(expecteds).flatten
    expecteds.zip(asts).each do |expected, ast|
      assert_equal(expected, ast.to_xpath(visitor))
    end
  end
  ...
  it "raises an exception on single quote" do
    assert_raises(Nokogiri::CSS::SyntaxError) { Nokogiri::CSS::Parser.new("'").parse }
  end

  it "raises an exception on invalid CSS syntax" do
    assert_raises(Nokogiri::CSS::SyntaxError) { Nokogiri::CSS::Parser.new("a[x=]").parse }
  end
  ...
  describe "selectors" do
    ...
    describe "attribute" do
      ...
      it "namespacing" do
        ...
        assert_xpath("//xmlns:a[@class='bar']", "a[class='bar']", ns: namespaces)
        assert_xpath("//xmlns:a[@hoge:class='bar']", "a[hoge|class='bar']", ns: namespaces)
      end
  ...
  describe "pseudo-classes" do
    ...
    it "raises an exception for pseudo-classes that are not XPath Names" do
      ...
      assert_raises(Nokogiri::CSS::SyntaxError) { Nokogiri::CSS::Parser.new("div:-moz-drag-over").parse }
    end
  end
  ...
  describe "functions" do
    ...
    it "supports custom functions" do
      ...
      ast = Nokogiri::CSS::Parser.new("a:aaron()").parse.first
