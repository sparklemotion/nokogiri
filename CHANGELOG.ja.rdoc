...
* 新しい機能
  ...
  * Nokogiri::XML::NodeSet now supports more Array and Enumerable operators:
    ...
    push, pop, shift, ==
  * Nokogiri::XML::NodeSet はもっと Array と Enumerable のオペレータを
    サポート出来る: index, delete, slice, - (相違), + (連結する),
    & (交差), push, pop, shift, ==
  ...
  * Nokogiri::XML::Node#namespace は Nokogiri::XML::Namespace を返す

  * Nokogiri::XML::Node#namespace= for setting a node's namespace
  * Nokogiri::XML::Node#namespace= ノードの名前空間をセット出来る

  * JRuby 1.3.0 support via FFI.
  * FFI で JRuby 1.3.0 はサポート出来ます
