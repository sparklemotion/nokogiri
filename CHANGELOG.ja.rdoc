...
* 新しい機能
  * Node#at_xpath はXPathの表現に合うNodeSetの最初の要素に返す
  * Node#at_css はCSSのセレクターに合うNodeSetの最初の要素に返す
  * NodeSet#| 合併させる為のもの GH #119 (Serabe ありがとう！)
  * NodeSet#inspect inspectは出力をきれいにするようになった
  * Node#inspect インスペクト後、普通のrubyで出力されるようになった
  * XML::DTD#external_id を追加
  * XML::DTD#system_id を追加
  * XML::ElementContent DTD要素のコンテンツを有効にするようになった
  * Nokogiri::XML::Builder内でのnamespace宣言用のサポートを改良
  * XML::Node#external_subsetを追加
  * XML::Node#create_external_subsetを追加
  * XML::Node#create_internal_subsetを追加
  * XML Builderは生成されていないstringsを付加出来るようになった
...
* バグの修正
  ...
  * Node#inner_html がNode#to_html と同じ独立変数を受け入れるようになった
    GH #117
  * DocumentFragment#css は子ノードに委任が出来る様になった GH #123
  * NodeSet#[]がNodeSet#lengthより大きい断片でも機能できる様になった　 GH #131
  * 新たな親ノードの名前空間の維持　GH #134
  * XML::Document をNodeSetに追加時のSEGVを修正
  * XML::SyntaxError が重複できるようになった
