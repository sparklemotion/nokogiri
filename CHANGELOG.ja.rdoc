...
* 機能
  ...
  * (MRI) 同梱ライブラリのビルド後に不要になったディレクトリ群を自動的に削除するようになった (デバッグ等の目的で残すようにするにはextconf.rbに --disable-clean を渡す) #952OB
  ...
  * Nokogiri::HTML::Document#title= は、head要素がない場合でも常に、最適な場所を探しつつtitle要素を追加するようになった
