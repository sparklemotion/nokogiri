...
static int io_read_callback(void * ctx, char * buffer, int len) {
  VALUE io = (VALUE)ctx;
  ...
  memcpy(buffer, StringValuePtr(string), NUM2INT(length));

  return NUM2INT(rb_funcall(string, rb_intern("length"), 0));
}

static int io_close_callback(void * ctx) {
  return 0;
}
