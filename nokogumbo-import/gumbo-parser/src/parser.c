...
static InsertionLocation get_appropriate_insertion_location (
  ...
  if (!parser->_parser_state->_foster_parent_insertions ||
      !node_tag_in_set(retval.target, (TagSet){TAG(TABLE), TAG(TBODY),
...
static void clear_stack_to_table_body_context(GumboParser* parser) {
  while (!node_tag_in_set(get_current_node(parser),
             (TagSet){TAG(HTML), TAG(TBODY), TAG(TFOOT), TAG(THEAD),
                 TAG(TEMPLATE)})) {
    pop_current_node(parser);
  }
}
...
static bool handle_before_html(GumboParser* parser, GumboToken* token) {
  if (token->type == GUMBO_TOKEN_DOCTYPE) {
    ...
    return true;
             ...
             !tag_in(token, false,
                 (TagSet){TAG(HEAD), TAG(BODY), TAG(HTML), TAG(BR)})) {
    parser_add_parse_error(parser, token);
    ignore_token(parser);
    return false;
  } else {
...
static bool handle_in_head(GumboParser* parser, GumboToken* token) {
  if (token->type == GUMBO_TOKEN_WHITESPACE) {
    ...
    return handle_in_body(parser, token);
                 ...
                 (TagSet){TAG(BASE), TAG(BASEFONT), TAG(BGSOUND),
    ...
    tag_in(token, kStartTag, (TagSet){TAG(NOFRAMES), TAG(STYLE)})
  ) {
    ...
    tag_in(token, kEndTag, (TagSet){TAG(BODY), TAG(HTML), TAG(BR)})
  ) {
    pop_current_node(parser);
    set_insertion_mode(parser, GUMBO_INSERTION_MODE_AFTER_HEAD);
    parser->_parser_state->_reprocess_current_token = true;
...
static bool handle_in_head_noscript(GumboParser* parser, GumboToken* token) {
  if (token->type == GUMBO_TOKEN_DOCTYPE) {
    ...
    tag_in(token, kStartTag, (TagSet){TAG(HEAD), TAG(NOSCRIPT)})
    || (
      token->type == GUMBO_TOKEN_END_TAG
...
static bool handle_in_body(GumboParser* parser, GumboToken* token) {
  ...
  if (token->type == GUMBO_TOKEN_NULL) {
    ...
    for (unsigned int i = 0; i < state->_open_elements.length; ++i) {
      if (!node_tag_in_set(state->_open_elements.data[i],
              (TagSet){TAG(DD), TAG(DT), TAG(LI), TAG(P), TAG(TBODY),
    ...
    return true;
  } else if (tag_in(token, kEndTag, (TagSet){TAG(BODY), TAG(HTML)})) {
    ...
    for (unsigned int i = 0; i < state->_open_elements.length; ++i) {
      if (!node_tag_in_set(state->_open_elements.data[i],
              (TagSet){TAG(DD), TAG(DT), TAG(LI), TAG(OPTGROUP),
    ...
    return success;
                 ...
                 (TagSet){TAG(ADDRESS), TAG(ARTICLE), TAG(ASIDE),
    ...
    insert_element_from_token(parser, token);
    return result;
  } else if (tag_in(token, kStartTag, (TagSet){TAG(H1), TAG(H2), TAG(H3),
                                          TAG(H4), TAG(H5), TAG(H6)})) {
    bool result = maybe_implicitly_close_p_tag(parser, token);
    if (node_tag_in_set(
            get_current_node(parser), (TagSet){TAG(H1), TAG(H2), TAG(H3),
                                          TAG(H4), TAG(H5), TAG(H6)})) {
      parser_add_parse_error(parser, token);
      pop_current_node(parser);
      result = false;
    ...
    insert_element_from_token(parser, token);
    return result;
  } else if (tag_in(token, kStartTag, (TagSet){TAG(PRE), TAG(LISTING)})) {
    bool result = maybe_implicitly_close_p_tag(parser, token);
    insert_element_from_token(parser, token);
  ...
  } else if (tag_in(token, kEndTag, (TagSet){TAG(H1), TAG(H2), TAG(H3),
                                        TAG(H4), TAG(H5), TAG(H6)})) {
    if (!has_an_element_in_scope_with_tagname(
      ...
      do {
        ...
        !node_tag_in_set (
          current_node,
          (TagSet){TAG(H1), TAG(H2), TAG(H3), TAG(H4), TAG(H5), TAG(H6)}
        )
      );
      return success;
    }
    ...
    return success;
                 ...
                 (TagSet){TAG(B), TAG(BIG), TAG(CODE), TAG(EM), TAG(FONT),
    ...
    return result;
                 ...
                 (TagSet){TAG(A), TAG(B), TAG(BIG), TAG(CODE), TAG(EM),
    ...
    tag_in(token, kStartTag, (TagSet){TAG(APPLET), TAG(MARQUEE), TAG(OBJECT)})
  ) {
    reconstruct_active_formatting_elements(parser);
    insert_element_from_token(parser, token);
    add_formatting_element(parser, &kActiveFormattingScopeMarker);
    ...
    tag_in(token, kEndTag, (TagSet){TAG(APPLET), TAG(MARQUEE), TAG(OBJECT)})
  ) {
    GumboTag token_tag = token->v.end_tag;
    ...
    return true;
                 ...
                 (TagSet){TAG(AREA), TAG(BR), TAG(EMBED), TAG(IMG),
    ...
    tag_in(token, kStartTag, (TagSet){TAG(PARAM), TAG(SOURCE), TAG(TRACK)})
  ) {
    insert_element_from_token(parser, token);
    pop_current_node(parser);
    acknowledge_self_closing_tag(parser);
    ...
    tag_in(token, kStartTag, (TagSet){TAG(OPTION), TAG(OPTGROUP)})
  ) {
    ...
    tag_in(token, kStartTag, (TagSet){TAG(RB), TAG(RP), TAG(RT), TAG(RTC)})
  ) {
    bool success = true;
    GumboTag exception =
      tag_in(token, kStartTag, (TagSet){TAG(RT), TAG(RP)})
        ? GUMBO_TAG_RTC
        : GUMBO_TAG_LAST;
    ...
    return true;
                 ...
                 (TagSet){TAG(CAPTION), TAG(COL), TAG(COLGROUP),
...
static bool handle_in_table(GumboParser* parser, GumboToken* token) {
  ...
  if (
    ...
    return true;
                 ...
                 (TagSet){TAG(TBODY), TAG(TFOOT), TAG(THEAD), TAG(TD),
                     TAG(TH), TAG(TR)})) {
    clear_stack_to_table_context(parser);
    set_insertion_mode(parser, GUMBO_INSERTION_MODE_IN_TABLE_BODY);
    if (tag_in(token, kStartTag, (TagSet){TAG(TD), TAG(TH), TAG(TR)})) {
      insert_element_of_tag_type (
        parser,
        GUMBO_TAG_TBODY,
        GUMBO_INSERTION_IMPLIED
    ...
    return true;
                 ...
                 (TagSet){TAG(BODY), TAG(CAPTION), TAG(COL),
    ...
    tag_in(token, kStartTag, (TagSet){TAG(STYLE), TAG(SCRIPT), TAG(TEMPLATE)})
    || (tag_is(token, kEndTag, GUMBO_TAG_TEMPLATE))
  ) {
    return handle_in_head(parser, token);
  } else if (
...
static bool handle_in_caption(GumboParser* parser, GumboToken* token) {
  if (tag_is(token, kEndTag, GUMBO_TAG_CAPTION)) {
    ...
    return true;
                 ...
                 (TagSet){TAG(BODY), TAG(COL), TAG(COLGROUP), TAG(HTML),
                     TAG(TBODY), TAG(TD), TAG(TFOOT), TAG(TH), TAG(THEAD),
                     TAG(TR)})) {
    parser_add_parse_error(parser, token);
    ignore_token(parser);
...
static bool handle_in_table_body(GumboParser* parser, GumboToken* token) {
  if (tag_is(token, kStartTag, GUMBO_TAG_TR)) {
    ...
    return false;
                 ...
                 (TagSet){TAG(TBODY), TAG(TFOOT), TAG(THEAD)})) {
    ...
    return true;
                 ...
                 (TagSet){TAG(BODY), TAG(CAPTION), TAG(COL), TAG(TR),
...
static bool handle_in_cell(GumboParser* parser, GumboToken* token) {
  ...
  } else if (tag_in(token, kEndTag, (TagSet){TAG(BODY), TAG(CAPTION),
                                        TAG(COL), TAG(COLGROUP), TAG(HTML)})) {
    parser_add_parse_error(parser, token);
    ignore_token(parser);
    return false;
  } else if (tag_in(token, kEndTag, (TagSet){TAG(TABLE), TAG(TBODY),
                                        TAG(TFOOT), TAG(THEAD), TAG(TR)})) {
    if (!has_an_element_in_table_scope(parser, token->v.end_tag)) {
      parser_add_parse_error(parser, token);
      ignore_token(parser);
...
static bool handle_in_select(GumboParser* parser, GumboToken* token) {
  if (token->type == GUMBO_TOKEN_NULL) {
    ...
    return false;
                 ...
                 (TagSet){TAG(INPUT), TAG(KEYGEN), TAG(TEXTAREA)})) {
    parser_add_parse_error(parser, token);
    if (!has_an_element_in_select_scope(parser, GUMBO_TAG_SELECT)) {
      ignore_token(parser);
    } else {
    ...
    return false;
                 ...
                 (TagSet){TAG(SCRIPT), TAG(TEMPLATE)}) ||
             tag_is(token, kEndTag, GUMBO_TAG_TEMPLATE)) {
    return handle_in_head(parser, token);
  } else if (token->type == GUMBO_TOKEN_EOF) {
    return handle_in_body(parser, token);
...
static bool handle_in_select_in_table(GumboParser* parser, GumboToken* token) {
  if (tag_in(token, kStartTag,
          (TagSet){TAG(CAPTION), TAG(TABLE), TAG(TBODY), TAG(TFOOT),
    ...
    return false;
                 ...
                 (TagSet){TAG(CAPTION), TAG(TABLE), TAG(TBODY),
...
static bool handle_in_template(GumboParser* parser, GumboToken* token) {
  ...
  if (token->type == GUMBO_TOKEN_WHITESPACE ||
    ...
    return handle_in_head(parser, token);
                 ...
                 token, kStartTag, (TagSet){TAG(CAPTION), TAG(COLGROUP),
                                       TAG(TBODY), TAG(TFOOT), TAG(THEAD)})) {
    pop_template_insertion_mode(parser);
...
static bool handle_in_foreign_content(GumboParser* parser, GumboToken* token) {
  ...
  if (tag_in(token, kStartTag,
          (TagSet){TAG(B), TAG(BIG), TAG(BLOCKQUOTE), TAG(BODY), TAG(BR),
          ...
          (token->type == GUMBO_TOKEN_CHARACTER ||
              ...
              (token->type == GUMBO_TOKEN_START_TAG &&
                  !tag_in(token, kStartTag,
                      (TagSet){TAG(MGLYPH), TAG(MALIGNMARK)})))) ||
